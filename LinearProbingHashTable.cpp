...
TEST("Stress test: Core functions do not cause stack overflows (should take at most 15 seconds)") {
    ...
    /* Insert the value 100000. This wants to go into slot zero, but that's filled, so it will
    ...
    /* Check if 100000 is there, which requires scanning the whole table. */
    EXPECT(table.contains(to_string(kTableSize)));
    /* Remove 100000, which requires scanning the whole table. */
