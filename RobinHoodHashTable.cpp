 ...
 * RobinHoodHashTable with the integer variables
...
bool RobinHoodHashTable::insert(const string& elem) {
    // if duplicate or no space, return false
    if (size() == amtOfSpots || contains(elem)) {
        return false;
    }
    int location = hashFunc(elem);
    RobinHoodHashTable::Slot slot = {elem, 0};
    // find next empty slot or slot filled by elem w/ less distance
    while (elems[location].distance != EMPTY_SLOT) {
        if (slot.distance > elems[location].distance) {
            RobinHoodHashTable::Slot slotCopy = elems[location];
            elems[location] = slot;
            slot = slotCopy;
        }
        location++;
        // wrap around check
        if (location == amtOfSpots) {
            location = 0;
        }
        slot.distance++;
    }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int distance = 0;
    while (elems[location].distance != EMPTY_SLOT
           && elems[location].element != elem
           && distance <= elems[location].distance) {
        location++;
        distance++;
        if (location == amtOfSpots) {
            location = 0;
    ...
    if (elems[location].element == elem && elems[location].distance != EMPTY_SLOT) {
        return true;
    }
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int location = hashFunc(elem);
    while (elems[location].distance != EMPTY_SLOT && elems[location].element != elem) {
        location = (location + 1) % amtOfSpots;
    }
    elems[location].distance = EMPTY_SLOT;
    insertedAmt--;
    int nextPosition = (location + 1) % amtOfSpots;
    while (elems[nextPosition].distance != 0) {
        if (elems[nextPosition].distance != EMPTY_SLOT) {
            elems[location].element = elems[nextPosition].element;
            elems[location].distance = elems[nextPosition].distance - 1;
            elems[nextPosition].distance = EMPTY_SLOT;
            nextPosition = (nextPosition + 1) % amtOfSpots;
            location = (location + 1) % amtOfSpots;
        }
        else {
            break;
